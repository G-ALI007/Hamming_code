...
m = 5; % Number of parity bits for Hamming code
...
fD = 50;  
rayleighChan = comm.RayleighChannel( ...
    'SampleRate', fs, ... % Sampling rate (adjustable)
    'PathDelays', pathDelays, ... % Multipath delays
    'AveragePathGains', avgPathGains, ... % Relative power of each path
    'MaximumDopplerShift', fD); % Doppler shift in Hz

% Generate a ones vector (or any other signal)
onesVector = ones(Len,1);

% Filter the signal through the Rayleigh channel
hhh = rayleighChan(onesVector); 

% Now, channelOutput contains the signal after passing through the Rayleigh channel
t=onesVector.*1/fs;
plot(20*log10(abs(hhh)))
title('Impulse response of Rayleigh channel in t2');
xlabel('sample')
ylabel('Amplitude')
grid on;
...
% Define Interleaver
p = randperm(14); % Permutation vector
dd=linspace(1,14,14);

interleavedData = intrlv(dd, p);
bb = deintrlv(interleavedData,p) % Deinterleave a to restore ordering.
